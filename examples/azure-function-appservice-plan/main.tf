...
locals {
  ...
  default_host_name                      = azurerm_linux_function_app.example.default_hostname
...
resource "azurerm_linux_function_app" "example" {
  ...
  # Azure Functions Storage Account Setting
  storage_account_name       = azurerm_storage_account.example.name
  # storage_account_access_key = azurerm_storage_account.example.primary_access_key
  storage_uses_managed_identity = true
  service_plan_id            = azurerm_service_plan.example.id

  # Azure Functions - Application Stack
  site_config {
    application_stack {
      node_version = "22"
    }
...
resource "azurerm_private_endpoint" "function_app" {
  name                = "pe-${azurerm_linux_function_app.example.name}"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name
  subnet_id           = azurerm_subnet.snet_internal_load_balancer.id

  private_service_connection {
    name                           = "psc-${azurerm_linux_function_app.example.name}"
...
resource "azurerm_private_dns_a_record" "function_app_scm" {
  name                = "${azurerm_linux_function_app.example.name}.scm"
  zone_name           = azurerm_private_dns_zone.example.name
  resource_group_name = azurerm_resource_group.example.name
  ttl                 = 300
  records             = [azurerm_private_endpoint.function_app.private_service_connection[0].private_ip_address]
